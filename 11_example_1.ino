...
#define SCALE (0.001 * 0.5 * SND_VEL) // coefficent to convert duration to 3distance
...
void loop() {
  ...
  if (dist_raw <= _TARGET_LOW){
    ...
    int duty_change = _DUTY_MIN9 + (dist_raw - _DIST_MIN) * (_DUTY_MAX - _DUTY_MIN) / (_DIST_MAX - _DIST_MIN);
    myservo.writeMicroseconds(duty_change);
  } else{
